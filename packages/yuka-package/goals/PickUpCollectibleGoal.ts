...
export class PickUpCollectibleGoal extends Goal<any> {
    ...
    override activate(): void {
        ...
        if (owner.components.has(componentRegistry.State)) {
            owner.components.write(componentRegistry.State).state = "Lifting";
        } else {
            owner.components.add(componentRegistry.State, {value: "Lifting"});
        }
    ...
    override execute(): void {
        ...
        if (targetEntity.has(componentRegistry.Packed)) {
        ...
        if (this.regulator.ready()) {
            ...
            if (this.stage > 1) {
                owner.sendMessage(this.target, (sender: any, receiver: any) => {
                    receiver.components.add(componentRegistry.Packed, {
