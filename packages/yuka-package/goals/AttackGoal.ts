...
//
// }
// applyMixins(GoalSubject, [Vehicle, Attacker, InWorld])
...
export class AttackGoal extends CompositeGoal<GoalSubject> {
    ...
    activate() {
        ...
        if (isTargetShootable(target) === true) {
            ...
            if (owner.world.PathPlanner.canMoveInDirection(owner, left, targetPosition)) {

                this.addSubgoal(new DodgeGoal(owner, false));

            } else if (owner.world.PathPlanner.canMoveInDirection(owner, right, targetPosition)) {
