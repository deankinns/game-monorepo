...
export class SkeletonTool {
  ...
  static async loadObj(
    factory: { load: { fbx: (arg0: string) => any } },
    key: string
    ...
    if (!object) {
      ...
      // }
      // @ts-ignore
      object = await factory.load.fbx(
        new URL(`./assets/fbx/${key}.fbx`, import.meta.url).toString()
      );
    ...
    clonedMesh.animations = await object.animations.map(
      (e: { clone: () => any }) => e.clone()
    );
  ...
  static getObj(key: string): ExtendedObject3D {
    ...
    const clonedMesh = SkeletonUtils.clone(object);
    // console.log(key, object.animations);
    clonedMesh.animations = object.animations.map((e: { clone: () => any }) =>
      e.clone()
    );
    return clonedMesh;
  }
