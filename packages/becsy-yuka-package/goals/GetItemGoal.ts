...
import {CONFIG, Vector3ToYuka} from "yuka-package/core";
import {PickUpCollectibleGoal} from "./PickUpCollectibleGoal";
import {Vehicle, GameEntity} from "../entities";
import {Collectable, Packed, PositionComponent, Target} from "becsy-package";
    ...
    getClosestCollectable(owner: Vehicle) {
        ...
        for (const entity of owner.manager?.entities as GameEntity[]) {
            ...
            if (
                ...
                entity.components.has(Collectable) &&
    ...
    execute() {
        ...
        if (this.active()) {
            ...
            if (currentSubgoal instanceof FollowPathGoal) {
                const tagertpos = target.read(PositionComponent).position;
